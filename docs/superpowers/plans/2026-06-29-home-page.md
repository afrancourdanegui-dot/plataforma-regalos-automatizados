...
**Goal:** Reemplazar el placeholder de `/` con la home page completa de Siempre Presente — nav, hero, cómo funciona, catálogo de categorías, CTA final y footer de 4 columnas.

**Architecture:** Un único Server Component `page.tsx` sin lógica de cliente. Todo el estilo vía clases Tailwind v4 (tokens ya en globals.css como `bg-arena`, `text-ciruelo`, etc.) con `style` prop para sombras y detalles que Tailwind no cubre limpiamente. Fotos via `next/image` con dominio Unsplash configurado en `next.config.ts`.

**Tech Stack:** Next.js 16 App Router, Tailwind v4, `next/image`, Google Fonts ya cargados en layout (Fraunces + Plus Jakarta Sans).

---

### Task 1: Habilitar imágenes de Unsplash en next/image

**Files:**
- Modify: `apps/web/next.config.ts`

- [ ] **Step 1: Agregar remotePatterns para images.unsplash.com**

Reemplazar el contenido de `apps/web/next.config.ts`:
...
- [ ] **Step 2: Verificar que el servidor de desarrollo arranca sin error**
...
Esperado: compilación exitosa, sin errores de configuración.
...
### Task 2: Implementar la home page completa

**Files:**
- Modify: `apps/web/src/app/page.tsx`

- [ ] **Step 1: Reemplazar page.tsx con la home completa**
  ...
  return (
    <div className="min-h-screen bg-crema font-sans text-carbon">
      ...
      <section className="mx-auto grid max-w-[1080px] grid-cols-1 items-center gap-8 px-6 pb-16 pt-8 md:grid-cols-2 md:gap-16">
        ...
        <div className="flex flex-col gap-3 md:order-last">
          {productCards.map((p) => (
            <div
              key={p.name}
              className={`flex items-center gap-3.5 rounded-[18px] border bg-white p-2.5 ${
                p.featured
                  ? "-translate-x-1 border-terracota"
                  : "border-arena"
              }`}
              style={{
      ...
      <section id="como-funciona" className="bg-arena">
        <div className="mx-auto max-w-[1080px] px-6 py-16">
          <div className="mb-12 text-center">
            <span className="mb-3.5 inline-flex items-center gap-1.5 rounded-full bg-white px-3.5 py-[7px] text-[11px] font-bold uppercase tracking-[0.06em] text-[#a8431f]">
              <span className="h-1.5 w-1.5 rounded-full bg-terracota" />
              Proceso
            </span>
      ...
      <section id="categorias" className="mx-auto max-w-[1080px] px-6 py-16">
        <div className="mb-12 text-center">
          <span className="mb-3.5 inline-flex items-center gap-1.5 rounded-full bg-arena px-3.5 py-[7px] text-[11px] font-bold uppercase tracking-[0.06em] text-[#a8431f]">
            <span className="h-1.5 w-1.5 rounded-full bg-terracota" />
            Catálogo
          </span>
          <h2 className="mt-3.5 font-display italic font-light text-[2rem] text-ciruelo">
...
Con el dev server corriendo (`npm run dev`), abrir `http://localhost:3000` y verificar:
- Nav aparece con logo y botones
- Hero muestra headline dos tonos + 3 tarjetas de producto con fotos
- Sección "Cómo funciona" tiene fondo arena con 3 tarjetas blancas
- Sección "Para cada ocasión" muestra 6 fotos de categorías en grid 3×2
- CTA final tiene fondo ciruelo oscuro
- Footer muestra 4 columnas (en desktop) con "próximamente" en las columnas Sobre/Legal
- Responsive: en móvil el hero apila en 1 columna, el footer en 2 columnas luego 1

Si las fotos no cargan → confirmar que `next.config.ts` fue guardado y el dev server fue reiniciado.
...
git commit -m "feat: implement home page with hero, steps, categories and footer"
git push origin main
```

Vercel desplegará automáticamente desde el push a main.
...
**Spec coverage:**
- [x] Nav: logo + login + crear cuenta ✓
- [x] Hero: eyebrow pill, h1 dos tonos, lead, CTAs, 3 product cards con fotos sin marca ✓
- [x] Primera tarjeta destacada con `border-terracota` + sombra + `-translate-x-1` ✓
- [x] "Cómo funciona" en banda arena, id `#como-funciona`, 3 tarjetas blancas, número en cuadrado ciruelo ✓
- [x] "Para cada ocasión" id `#categorias`, 6 fotos en grid, sin precios ✓
- [x] CTA Final fondo ciruelo + círculo decorativo durazno ✓
- [x] Footer 4 columnas responsive (1→2→4) ✓
- [x] Items "próximamente" son `<span>`, no `<a>` ✓
- [x] Restricciones honestidad: sin marca en hero cards, sin testimonios, sin logos de marca ✓
- [x] `next.config.ts` con remotePatterns Unsplash ✓

**Placeholder scan:** ninguno — todo el código está completo en los steps.

**Type consistency:** solo se usan tipos primitivos y arrays de literales, sin interfaces cross-task.
